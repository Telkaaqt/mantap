    ...
    async def wrapper(client, message):
        ...
        if message.command[0][0] == "v":
            ...
            if "-v" in message.text:
                ...
                video = False if message.command[0][1] == "v" else False
...
def CPlayWrapper(command):
    async def wrapper(client, message):
        ...
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(
                    text=f"{i.mention} ɪs ᴜɴᴅᴇʀ ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ, ᴠɪsɪᴛ <a href={SUPPORT_GROUP}>sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ</a> ғᴏʀ ᴋɴᴏᴡɪɴɢ ᴛʜᴇ ʀᴇᴀsᴏɴ.",
                    disable_web_page_preview=True,
                )
        ...
        if playty != "Everyone":
            if message.from_user.id not in SUDOERS:
                admins = adminlist.get(message.chat.id)
                if not admins:
                    return await message.reply_text(_["speed_10"])
                else:
                    if message.from_user.id not in admins:
                        return await message.reply_text(_["play_4"])
        if message.command[0][0] == "v":
        ...
        if not await is_active_chat(chat_id):
            ...
            try:
                try:
                    ...
                    await message.reply_text(_["call_4"])
                if (
                    get.status == ChatMemberStatus.BANNED
                    or get.status == ChatMemberStatus.RESTRICTED
                ):
                ...
                if chat_id in clinks:
                    ...
                    if message.chat.username:
                        ...
                        try:
                            ...
                            await message.reply_text(
                                _["call_9"].format(i.mention, type(e).__name__)
                            )
                ...
                myu = await message.reply_text(_["call_5"].format(i.mention))
                try:
                    await asyncio.sleep(1)
                    await userbot.join_chat(invitelink)
                except InviteRequestSent:
                    ...
                    await myu.edit(_["call_6"].format(i.mention))
                except UserAlreadyParticipant:
                    pass
                except Exception as e:
                    await message.reply_text(
